  ...
  //        BasicBlock* bb = L->getUniqueExitBlock();  
    //      errs()<<bb->getName()<<" Exit block name \n";
     ...
     bool inSubLoop(BasicBlock *bb)
     {
      ...
      assert(CurrentLoop->contains(bb) && "Valid if BB is in loop");
   ...
   bool isExecuted(Instruction &I){
       ...
       if (!DT->dominates(I.getParent(), ExitBlocks[i]))
        {
           //errs()<<"does not dom all exit block "<<ExitBlocks[i]->getName()<<"\n";
           return false;
        }
       if (ExitBlocks.empty())
         return false;
